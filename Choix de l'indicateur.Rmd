...
```{r from_json_to_df_function_error_msg}
from_json_to_df <- function(WD,
                            threshold = 0.6,
                            fox_id_limits = c(5,8),
                            date_time_limits = c(10,28),
  ...
  for (file in files) {
    if (grepl("\\.json$", file)) {
      tryCatch({
        data_loaded <- load_json(WD = WD, file_name = file)
        data_1line <- one_position_file(json_file = data_loaded,
                                        threshold = threshold,
                                        fox_id_limits = fox_id_limits,
                                        date_time_limits = date_time_limits,
        ...
        message(paste("❌ Erreur rencontrée avec le fichier :", file))
        message("🛠️ Détail de l'erreur :", e$message)
      })
    }
  }
