...
```{r}
# Distance walked by timestep

distance_walked <- function(data, interval_seconds) {
 ...
 data <- data %>% 
   rowwise() %>%
   mutate(movement_x = case_when(rownames(.)== "1" ~ 0,
                                rownames(.)!="1" ~
                                 data[rownames(.),]$relative_coordinates$center_x - data[as.numeric(rownames(.))-1,]$relative_coordinates$center_x),
          movement_y = case_when(rownames(.) == "1" ~ 0,
                                 rownames(.) != "1" ~ data[rownames(.),]$relative_coordinates$center_y - data[as.numeric(rownames(.))-1,]$relative_coordinates$center_y)
     )

  # df_counts <- df_counts %>%
  # rowwise() %>%        # Pour chaque ligne
  #   mutate(
  #     distance = 
...
df_distance <- distance_walked(data = data, interval_seconds = 3600)

data[data$Date_Time == "2023-01-30 11:56:13 CET",]
...
calculate_distances <- function(data, interval) {
  ...
  data <- data %>%
    ...
    filter(!is.na(distance) & time_diff == 1)
