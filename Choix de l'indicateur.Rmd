...
```{r}
# Conversion de la colonne Date_Time en format date-heure si nécessaire
df$Date_Time <- as.POSIXct(df$Date_Time, format = "%Y-%m-%d %H:%M:%S")

# Filtrage des positions valides
df_clean <- df %>%
  filter(!is.na(Position), Position != "not_visible") %>%
  arrange(Date_Time)

# Détection des changements de position
df_clean <- df_clean %>%
  mutate(Position_Change = Position != lag(Position, default = first(Position)))
