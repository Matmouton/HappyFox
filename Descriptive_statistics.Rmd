...
# A. Consistency of the certainty distribution

## 1. Confidence depending on the number of predictions
...
ggplot(merged_data, aes(x = group, y = confidence, fill = group)) +
  ...
  ggtitle("Data from 04/07/2022 to 14/07/2022",
...
load("data_8h_21h_05_07_to_08_08_2022.Rdata")

# New dataset with the number of predictions as a factor variable
data_group_number_pred <- data_8h_21h_05_07_to_08_08_2022 %>%
  mutate(group = as.factor(number_of_predictions))

# New dataset with a variable "group" always being "All"
data_group_overall <- data_8h_21h_05_07_to_08_08_2022 %>%
  mutate(group = "Total")
...
# When 1 prediction
hist((data_8h_21h_05_07_to_08_08_2022 %>% 
...
data_less_5pred <- data_8h_21h_05_07_to_08_08_2022 %>% 
...
## 2. Confidence gap depending on the number of predictions
...
group1 <- (data_8h_21h_05_07_to_08_08_2022 %>% #2 pred
  filter(number_of_predictions == 2) %>% 
  select(confidence_gap))[[1]]

group2 <- (data_8h_21h_05_07_to_08_08_2022 %>% #3 pred
...
# B. Comparison with or without no reliable predictions
...
data_8h_21h_05_07_to_08_08_2022 <- data_8h_21h_04_07_to_08_08_2022 %>% 
  filter(date != "2022-07-04")

# Test for 1h

frequency_position_05_07_to_08_08_2022 <- frequency_position(data = data_8h_21h_05_07_to_08_08_2022, interval_seconds = 900, existing_data_percent = 95)


frequency_position_05_07_to_08_08_2022_PIVOT <- frequency_position_05_07_to_08_08_2022[,-2] %>% #drop the end_time column
  pivot_longer(
    cols = !start_time, 
    names_to = "Posture", 
    values_to = "Count"
...
frequency_position_05_07_to_08_08_2022_PIVOT <- pivot_data(data = frequency_position_05_07_to_08_08_2022,
 ...
 # Test
frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT <- filter_8h_21h(frequency_position_05_07_to_08_08_2022_PIVOT, start_time)

rm(frequency_position_05_07_to_08_08_2022_PIVOT)  #remove data with the 21h-8h gap


# Test

frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time <- add_compressed_time_variable_PIVOT_data(frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT, nbrep = 4) #4 postures



#remove useless data : no compressed time
rm(frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT)
```
...
reliable_frequency_position_05_07_to_08_08_2022 <- frequency_position(data = reliable_data_8h_21h_05_07_to_08_08_2022, interval_seconds = 900, existing_data_percent = 95)

#Pivot data
reliable_frequency_position_05_07_to_08_08_2022_PIVOT <- reliable_frequency_position_05_07_to_08_08_2022[,-2] %>% #drop the end_time column
  pivot_longer(
    cols = !start_time, 
    names_to = "Posture", 
    values_to = "Count"
...
reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT <- filter_8h_21h(reliable_frequency_position_05_07_to_08_08_2022_PIVOT, start_time)

rm(reliable_frequency_position_05_07_to_08_08_2022_PIVOT)  #remove data with the 21h-8h gap

#Add compressed time variable
reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time <- add_compressed_time_variable_PIVOT_data(reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT, nbrep = 4) #4 postures

#remove useless data : no compressed time
rm(reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT)
...
plot1 <- ggplot(frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time, aes(x = Posture, y = Count, fill = Posture)) +
  geom_boxplot() +
  xlab("Posture") +
  ylab("Amount of occurences every 15 minutes") +
  theme(legend.position = "right")

# Without unreliable predictions
plot2 <- ggplot(reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time, aes(x = Posture, y = Count, fill = Posture)) +
  geom_boxplot() +
  xlab("Posture") +
  ylab("Amount of occurences every 15 minutes") +
  theme(legend.position = "right")
...
We compute a two-sample Kolmogorov-Smirnov test to determine if the amount of
every posture during a timestep of 15min follows the same distribution. Results
: same distributions (for each), because p-value \> 0,05 = no differences.

```{r}
# lying
all_data <- (frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time %>%
  filter(Posture == "lying") %>% 
  select(Count))[[1]]

reliable_data <- (reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time %>% 
  filter(Posture == "lying") %>% 
  select(Count))[[1]]

ks.test(all_data, reliable_data)
...
Illustration for posture not_visible :

```{r}
hist((frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time %>% 
       filter(Posture == "not_visible") %>% #when 3 predictions made
       select(Count)                     #get the confidence
      )[[1]],                                 #only 1st object of the output
     main = "Count for posture not_visible",
     xlab = "Count",
...
hist((reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time %>% 
       filter(Posture == "not_visible") %>% #when 3 predictions made
       select(Count)                     #get the confidence
      )[[1]],                                 #only 1st object of the output
     main = "Count for posture not_visible",
...
```{r}
# Chart for count
ggplot(data = frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time,
...
```{r}
# Chart for count
ggplot(data = reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time,
       aes(fill = Posture, y = Count, x = compressed_time)) +         #start_time as x variable
  geom_bar(position = "stack", stat = "identity") +              #stacked bars, no %
  ggtitle("Fox4313's posture every 15min between days 04/07/2022 and 06/07/2022",
...
```{r}
from_data_to_plot(data_8h_21h = data_8h_21h_05_07_to_08_08_2022,
...
from_data_to_plot(data_8h_21h = reliable_data_8h_21h_05_07_to_08_08_2022,
                  interval_seconds = 3600,
                  title = "Posture's score of fox 4313",
                  subtitle = "Without non-reliable data, from 04/07/2022 to 14/07/2022",
                  x_lab = "Cumulative time of the day (only between 8h and 21h)",
                  y_lab = "Movement score by summing posture indices")
```

```{r}
grid.arrange(from_data_to_plot(data_8h_21h = data_8h_21h_05_07_to_08_08_2022,
                  interval_seconds = 3600), 
             from_data_to_plot(data_8h_21h = reliable_data_8h_21h_05_07_to_08_08_2022,
                  interval_seconds = 3600),
             ncol=2)
...
## 4. Indicator 3 : same distribution
...
amount_changes_data_8h_21h_05_07_to_08_08_2022 <- amount_posture_changes(data_8h_21h_05_07_to_08_08_2022, "15 minute")

# Skip parts between 21h and 8h
amount_changes_data_8h_21h_05_07_to_08_08_2022_compressed_time <- add_compressed_time_variable(amount_changes_data_8h_21h_05_07_to_08_08_2022)

# Plot
ggplot(amount_changes_data_8h_21h_05_07_to_08_08_2022_compressed_time,
  ...
  ggtitle("Posture changes of fox 4313", subtitle = "All data, from 04/07/2022 to 14/07/2022, every 15 minutes") +
...
# Changes per 15minutes & per day
amount_changes_data_8h_21h_05_07_to_08_08_2022_reliable_data <- amount_posture_changes(reliable_data_8h_21h_05_07_to_08_08_2022, "15 minute")

# Skip parts between 21h and 8h
amount_changes_data_8h_21h_05_07_to_08_08_2022_compressed_time_reliable_data <- add_compressed_time_variable(amount_changes_data_8h_21h_05_07_to_08_08_2022_reliable_data)

# Plot
ggplot(amount_changes_data_8h_21h_05_07_to_08_08_2022_compressed_time_reliable_data,
       aes(x = compressed_time, y = Nb_changes)) +
  ...
  #xlab("Temps entre 8h et 21h chaque jour") +
  ylab("Amount of posture changes") +
  ggtitle("Posture changes of fox 4313", subtitle = "Only reliable data, from 04/07/2022 to 14/07/2022, every 15 minutes") +
  scale_x_continuous(name = "Time between 8h and 21h each day",
                     breaks = c(0,50,100,150,200,250,300,350,400,450))
```

# C. Descriptive statistics for each indicator
...
#Calculate frequency position
reliable_frequency_position_05_07_to_08_08_2022 <- frequency_position(data = reliable_data_8h_21h_05_07_to_08_08_2022, interval_seconds = 3600, existing_data_percent = 50)
...
#Add compressed time variable
reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time <- add_compressed_time_variable_PIVOT_data(reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT, nbrep = 4)
...
```{r}
ggplot(reliable_frequency_position_8h_21h_05_07_to_08_08_2022_PIVOT_compressed_time %>% 
         filter(Posture == "not_visible"),
       #%>% filter(Count != 0),
       aes(x = Count, color = Posture)) +
...
ggplot(frequency_position_8h_21h_05_07_to_08_08_2022_StandingSitting_1h_PIVOT_compressed_time %>% 
       ...
       #%>% filter(Count != 0),
       aes(x = Count, color = Aggregated_postures)) +
  geom_histogram(aes(y = after_stat(density)), colour = "black", fill = "white") +
  geom_density(alpha = .2, fill = "#FF6666")
...
# Add the variable posture_123
data_05_07_to_08_08_2022_increasing_code <- posture_123(reliable_data_8h_21h_05_07_to_08_08_2022)

# Compute the amount of activity
amount_activity_1h_data_05_07_to_08_08_2022 <- amount_activity(data = data_05_07_to_08_08_2022_increasing_code,
                                     interval_seconds = 3600) 

# Filter between 8h and 21h
amount_activity_1h_8h_21h_data_05_07_to_08_08_2022 <- filter_8h_21h(
    data = amount_activity_1h_data_05_07_to_08_08_2022,
    time_column = interval_start)

# Add compressed_time variable
 amount_activity_1h_8h_21h_data_05_07_to_08_08_2022_compressed_time <- add_compressed_time_variable(amount_activity_1h_8h_21h_data_05_07_to_08_08_2022)
...
```{r}
ggplot(amount_activity_1h_8h_21h_data_05_07_to_08_08_2022_compressed_time, 
       #%>% filter(total_activity != 0), 
       aes(x = total_activity)) +
  geom_histogram(aes(y = after_stat(density)), colour = "black", fill = "white") +
  geom_density(alpha = .2, fill = "#FF6666")
```
...
# Get the dataset
amount_changes_1h_data_8h_21h_05_07_to_08_08_2022_compressed_time <- add_compressed_time_variable(amount_changes_1h_data_8h_21h_05_07_to_08_08_2022)
```

Plot :

```{r}
ggplot(amount_changes_1h_data_8h_21h_05_07_to_08_08_2022_compressed_time,
       #%>% filter(Nb_changes != 0),
       aes(x = Nb_changes)) +
  geom_histogram(aes(y = after_stat(density)), colour = "black", fill = "white") +
  geom_density(alpha = .2, fill = "#FF6666")
...
# D. Piechart postures calibration phase vs symptomatic phase

Piecharts of postures post-infection but without symptoms (= data from 09/07 to
14/07) compared to the symptomatic phase when moving a lot (05/08) & biting
himself while lying all day long (06/08) i.e. considered lying a lot.
...
### For "reference" period (09/07-14/07) :

# Get number of rows for these days
nrow(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
  filter(Date_Time >= as.POSIXct("2022-07-09 08:00:00") & Date_Time <= as.POSIXct("2022-07-14 21:00:00"))) #274105 rows

# Get the proportion of each posture on these days
table(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
        filter(Date_Time >= as.POSIXct("2022-07-09 08:00:00") & Date_Time <= as.POSIXct("2022-07-14 21:00:00")) %>%
        select(name))/274105  #divide by total to get proportion

# The remaining rows = NAs :
nrow(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
  filter(Date_Time >= as.POSIXct("2022-07-09 08:00:00") & Date_Time <= as.POSIXct("2022-07-14 21:00:00")) %>% 
  filter(is.na(name)))/274105  #64713 rows = 23,6%
...
nrow(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
  filter(date == "2022-08-05")) #45110 rows

# Get the proportion of each posture on these days
table(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
        filter(date == "2022-08-05") %>%
        select(name))/45110  #divide by total to get proportion

# The remaining rows = NAs :
nrow(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
  filter(date == "2022-08-05") %>% 
  filter(is.na(name)))/45110  #8135 rows = 18,0%
...
nrow(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
  filter(date == "2022-08-06")) #45032 rows

# Get the proportion of each posture on these days
table(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
        filter(date == "2022-08-06") %>%
        select(name))/45032  #divide by total to get proportion

# The remaining rows = NAs :
nrow(reliable_data_8h_21h_05_07_to_08_08_2022 %>% 
  filter(date == "2022-08-06") %>% 
  filter(is.na(name)))/45032  #64713 rows = 25,9%
```
...
# Data
  ...
  # Reference piechart (from 09/07 to 14/07)
group <- c("Lying", "Sitting", "Standing", "Not visible", "NA")
value <- c(52, 5, 9, 11, 23)    #values of the table, in % (rounded)

data_piechart_ref <- data.frame(group, value) %>%
  arrange(desc(group)) %>%
  mutate(
  ...
  # Very active day (05/08)
group <- c("Lying", "Sitting", "Standing", "Not visible", "NA")
value <- c(24, 7, 40, 11, 18)    #values of the table, in % (rounded)

data_piechart_05_08 <- data.frame(group, value) %>%
  arrange(desc(group)) %>%
  mutate(
    prop = value / sum(value),
    ypos = cumsum(prop) - 0.5 * prop
  ...
  # Identified as lying, but in fact biting himself while lied (06/08)
group <- c("Lying", "Sitting", "Standing", "Not visible", "NA")
value <- c(58, 1, 5, 10, 26)    #values of the table, in % (rounded)

data_piechart_06_08 <- data.frame(group, value) %>%
  arrange(desc(group)) %>%
  mutate(
    prop = value / sum(value),
    ypos = cumsum(prop) - 0.5 * prop
...
custom_colors <- c(
  ...
  "Sitting" = "orange",
  "Standing" = "tomato",
  "Not visible" = "darkseagreen", #in green because he's supposed to be lying
  "NA" = "lightgrey"
)

# Graph reference
ggplot(data_piechart_ref, aes(x = "", y = prop, fill = group)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  theme_void() +
  geom_text(aes(y = ypos, label = paste0(value)), color = "white", size = 6) +
  scale_fill_manual(values = custom_colors) +
  guides(fill = guide_legend(title = "Posture")) +
  ggtitle("Amount of each posture for fox 4313",
          subtitle = "During the infected pre-symptomatic phase (09/07-14/07)")

# Graph 05/08
ggplot(data_piechart_05_08, aes(x = "", y = prop, fill = group)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  theme_void() +
  geom_text(aes(y = ypos, label = paste0(value)), color = "white", size = 6) +
  ...
  guides(fill = guide_legend(title = "Posture")) +
  ggtitle("Amount of each posture for fox 4313",
          subtitle = "During the symptomatic phase, active day (05/08)")

# Graph 06/08
ggplot(data_piechart_06_08, aes(x = "", y = prop, fill = group)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y", start = 0) +
  theme_void() +
  geom_text(aes(y = ypos, label = paste0(value)), color = "white", size = 6) +
  ...
  guides(fill = guide_legend(title = "Posture"))+
  ggtitle("Amount of each posture for fox 4313",
          subtitle = "During the symptomatic phase, inactive day (06/08)")
```
