...
amount_posture_changes <- function(
  ...
  # # If more than 10 following NAs : don't consider these data
  # 
  #   # Set count to 0 & get data length
  # counter <- 0
  # n <- nrow(data)
  # 
  #   # Spot too much NAs
  # for (i in 1:n) {
  #   if (is.na(data$name[i])) {
  #     counter <- counter + 1
  #     if (counter > max_na) {
  #       data$data_no_NA[(i-max_na):i] <- NA
  #     }
  #   } else {
  #     counter <- 0  # Réinitialisation du compteur
  #   }
  # 
  #   # Si le compteur est déjà > 10, on continue à mettre NA
  #   if (counter > max_na) {
  #     reliable_data_8h_21h_05_07_to_08_08_2022$data_no_NA[i] <- NA
  #   }
  # }
...
changes_by_group_time <- function(data, time_gap) {
  data %>%
    mutate(Time_group = floor_date(Date_Time, unit = paste0(time_gap, " secs"))) %>% #make time groups
    group_by(Time_group) %>%
...
changes_by_group_time <- function(data, time_gap) {
  data %>%
    arrange(Date_Time) %>%
    mutate(Time_group = as.POSIXct(floor(as.numeric(Date_Time) / time_gap) * time_gap, origin = "1970-01-01")) %>%
    group_by(Time_group) %>%
    summarise(
      Nb_changes = sum(Position_change, na.rm = TRUE),
      Nb_NA = sum(is.na(name))
    ) %>%
    ungroup()
}
...
amount_posture_changes(reliable_data_8h_21h_05_07_to_08_08_2022, "1 day")
```
