...
data_prep_xbarchart_ggplot <- function(data_calibration,
  ...
  qcc_data$Out_1sd <- with(qcc_data, Value > q$center+SD | Value < q$center-SD)
    ...
    # Add conditions for rule2 (2/3 following points > 2sigma from the center)
  
        # Check if each point is at <1sd distance from mean + check if up/down
  qcc_data <- qcc_data %>%
    mutate(more_2sd_distance = case_when(
      Value > q$center + 2*SD ~ 1,  #more than 2sd distance from mean, positive side
      Value < q$center - 2*SD ~ -1, #more than 2sd distance from mean, negative side
      TRUE ~ 0                  #within [mean +/- 2sd]
    )) %>%
        # Check if 2/3 following points are >2sd distance from mean on the same side
    mutate(Rule2 = slide_int(more_2sd_distance, ~{
      pos <- sum(.x == 1)       #sum the >2 sd positive side
      neg <- sum(.x == -1)      #sum the >2 sd negative side
      if(pos >= 2 | neg >= 2) TRUE else FALSE  #T if 2 points >2 sd on the same side
    }, .before = 2,             #we watch the 2 previous points (+ the current point)
    .complete = TRUE))          #do nothing (= NA) if there are not 2 previous points

  
    # Add conditions for rule3 (6 following points are increasing/ decreasing : trend)
  ...
  qcc_data <- qcc_data %>% 
    ...
    mutate(Rule3 = slide_int(increase_decrease, ~{
      inc <- sum(.x == 1)      #sum the increases
      dec <- sum(.x == -1)     #sum the decreases
      if( inc >= 6 | dec >= 6) TRUE else FALSE  #T if more than 6 neighbours inc/dec
    }, .before = 5,            #watch 5 previous points (+ the current point)
    ...
    # Add conditions for rule4 (4/5 following points > 1sigma from the center)
  ...
  qcc_data <- qcc_data %>%
    mutate(more_1sd_distance = case_when(
      ...
      TRUE ~ 0                  #within [mean +/- 1sd]
        ...
        # Check if 4/5 following points are >1sd distance from mean on the same side
    mutate(Rule4 = slide_int(more_1sd_distance, ~{
      pos <- sum(.x == 1)       #sum the >1 sd positive side
      neg <- sum(.x == -1)      #sum the >1 sd negative side
      if(pos >= 4 | neg >= 4) TRUE else FALSE  #T if 4 points >1 sd on the same side
    }, .before = 4,             #we watch the 4 previous points (+ the current point)
  ...
  qcc_data <- qcc_data %>% 
    mutate(OutOfControl = case_when(
      ...
      # The 4 rules I want on my chart
      # from most to less important so that most important rules have the priority
      Out_3sd == TRUE ~ "Rule 1",
      Rule2 > 0 ~ "Rule 2",
      Rule3 > 0 ~ "Rule 3",
      Rule4 > 0 ~ "Rule 4",
...
plot_xbarchart_ggplot <- function(data_calibration,
  ...
  ggplot(qcc_data, aes(x = Date)) +
      # Lines
    geom_vline(xintercept=10.5, linetype='dashed', color='darkgrey', size=0.8) +  #missing data
    geom_vline(xintercept=11.5, linetype='dashed', color='darkgrey', size=0.8) +  #missing data
...
And finally, let's plot the graphs !

Reminder : timestep = 1hour for indicators :

-   1.1, postures lying + standing + not_visible

-   1.2

-   2

timestep = 30min for indicator 1.1, posture sitting

timestep = 2hours for indicator 3

```{r}
  # Indicator 1.1 (each posture separately)
load("data_SCP_with_QCC_indicator1.1_BEST_TIMESTEP_ONLY.RData")
...
plot_xbarchart_ggplot(data_calibration = lying_byday_1h[1:10,],
    ...
    # Sitting
plot_xbarchart_ggplot(data_calibration = sitting_byday_30min[1:10,],
                      data_test = sitting_byday_30min[c(30, 32:35),],
                      title = "Xbar chart for indicator 1.1 : Sitting",
                      subtitle = "Every 30 minutes, for fox 4313",
...
plot_xbarchart_ggplot(data_calibration = not_visible_byday_1h[1:10,],
  ...
  # Indicator 1.2 (standing_sitting)
load("data_SCP_with_QCC_indicator1.2.RData")

plot_xbarchart_ggplot(data_calibration = sitting_standing_byday_1h[1:10,],
                      data_test = sitting_standing_byday_1h[c(30, 32:35),],
                      title = "Xbar chart for indicator 1.2 : sitting & standing",
                      subtitle = "Every hour, for fox 4313",
...
# Indicator 3 : amount of posture changes
load("data_SCP_with_QCC_indicator3.RData")

plot_xbarchart_ggplot(data_calibration = posture_changes_byday_2h[1:10,],
                      data_test = posture_changes_byday_2h[11:15,],
                      title = "Xbar chart for indicator 3 : posture changes by day",
                      subtitle = "Every 2 hours, for fox 4313",
